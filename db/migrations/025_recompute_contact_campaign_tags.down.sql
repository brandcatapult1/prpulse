-- Restore one-shot propagate-on-first-count behavior (pre-recompute invariant).
...
DROP FUNCTION IF EXISTS recompute_contacts_for_campaign_tags(uuid);
DROP FUNCTION IF EXISTS recompute_contact_campaign_tags(uuid);
