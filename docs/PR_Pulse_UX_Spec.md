...
- **→ Scheduled** (board hover / drawer): blocked unless visit date, ≥1 deliverable, and `primary_collaboration_reason` are set; error names whichever is missing (`transitionStage`).
...
- **→ Scheduled:** **opens a required Visit `Modal`** (Visit Date*, Time, Outlet, Notes). On save, `next_follow_up_date = visit_date`. Blocked unless visit date, ≥1 deliverable, and `primary_collaboration_reason` are set — message names whichever is missing (`transitionStage`).
...
- [ ] Setting `Scheduled` requires visit date, ≥1 deliverable, and collab reason; block message names missing prerequisites; enforced via shared `transitionStage`.
